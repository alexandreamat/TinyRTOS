...
typedef struct {
  const bool mini_uart_irq : 1;  // If set the mini UART has an
                                 // interrupt pending. RO 0x0
  const bool
      spi_1_irq : 1;  // If set the SPI1 module has an interrupt pending. RO 0x0
  const bool spi_2_irq : 1;  // If set the SPI 2 module has an
                             // interrupt pending. RO 0x0
} aux_irq_t;

// Auxiliary Interrupt status
#define AUX_IRQ ((const volatile aux_irq_t *const)(AUX_BASE + 0x00))

typedef struct {
  bool mini_uart_enable : 1;  // If set the mini UART is enabled. The UART will
                              // immediately start receiving data, especially if
                              // the UART1_RX line is low. If clear the mini
                              // UART is disabled. That also disables any mini
                              // UART register access
  bool spi1_enable : 1;  // If set the SPI 1 module is enabled. If clear the SPI
                         // 1 module is disabled. That also disables any SPI 1
                         // module register access
  bool spi2_enable : 1;  // If set the SPI 2 module is enabled. If clear the SPI
...
typedef union {
  const char receive_data_read : 8;  // DLAB=0 Data read is taken from the
                                     // receive FIFO (Provided it is not empty)
                                     // (Only if bit 7 of the line control
                                     // register (DLAB bit) is clear) RO 0x00
  char transmit_data_write : 8;  // DLAB=0 Data written is put in the transmit
                                 // FIFO (Provided it is not full) (Only if bit
                                 // 7 of the line control register (DLAB bit) is
                                 // clear) WO 0x00
  unsigned int
...
typedef struct {
  bool enable_transmit_interrupt : 1;  // (DLAB=0) If this bit is set the
                                       // interrupt line is asserted whenever
                                       // the transmit FIFO is empty. If this
                                       // bit is clear no transmit interrupts
                                       // are generated. RW 0x0
  bool enable_receive_interrupt : 1;   // (DLAB=0) If this bit is set the
...
typedef union {
  const struct {
    bool interrupt_not_pending : 1;  // This bit is clear whenever an
                                     // interrupt is pending RO 0x1
    uart_interrupt_id_bit_t
        interrupt_id_bits : 2;  // On read this register shows the interrupt ID
  } on_read;
  struct {
    int : 1;
    bool fifo_clear_rx : 1;  // Writing with bit 1 set will
                             // clear the receive FIFO
    bool fifo_clear_tx : 1;  // Writing with bit 2 set will
...
typedef struct {
  bool data_size_is_8 : 1;  // If clear the UART works in 7-bit mode If
                            // set the UART works in 8-bit mode RW 0x0
  int : 5;
  bool break_ : 1;     // If set high the UART1_TX line is pulled low
                       // continuously. If held for at least 12 bits times
                       // that will indicate a break condition. RW 0x0
  bool dlab_mode : 1;  // If set the first two Mini UART registers
                       // give access to the Baudrate register. During
                       // operation this bit must be cleared. RW 0x0
} aux_mu_lcr_reg_t;
...
typedef struct {
  int : 1;
  bool rts_is_low : 1;  // If clear the UART1_RTS line is high If set the
                        // UART1_RTS line is low This bit is ignored if the RTS
                        // is used for auto-flow control. See the Mini UART
                        // Extra Control register description) RW 0x0
} aux_mu_mcr_reg_t;
...
typedef struct {
  bool data_ready : 1;  // This bit is set if the receive FIFO
  ...
  int : 3;
  bool transmitter_empty : 1;  // This bit is set if the transmit FIFO
                               // can accept at least one byte. RO 0x0
  bool transmitter_idle : 1;   // This bit is set if the transmit FIFO is empty
...
#define AUX_MU_LSR_REG \
  ((const volatile aux_mu_lsr_reg_t *const)(AUX_BASE + 0x54))
...
typedef struct {
  bool receiver_enable : 1;     // If this bit is set the mini UART receiver is
                                // enabled. If this bit is clear the mini UART
                                // receiver is disabled RW 0x1
  bool transmitter_enable : 1;  // If this bit is set the mini UART transmitter
                                // is enabled. If this bit is clear the mini
                                // UART transmitter is disabled RW 0x1
  bool enable_rx_auto_flow_ctl_rts : 1;  // If this bit is set the RTS line will
                                         // de-assert if the receive FIFO
                                         // reaches its 'auto flow' level. In
                                         // fact the RTS line will behave as an
                                         // RTR (Ready To Receive) line. If this
                                         // bit is clear the RTS line is
                                         // controlled by the AUX_MU_MCR_REG
                                         // register bit 1. RW 0x0
  bool enable_tx_auto_flow_ctl_cts : 1;  // If this bit is set the transmitter
                                         // will stop if the CTS line is de-
                                         // asserted. If this bit is clear the
                                         // transmitter will ignore the status
                                         // of the CTS line RW 0x0
  rts_auto_flow_level_t
  ...
  bool cts_assert_level : 1;  // This bit allows one to invert the CTS auto flow
                              // operation polarity. If set the CTS auto flow
                              // assert level is low* If clear the CTS auto flow
                              // assert level is high* RW 0x0
  bool rts_assert_level_is_low : 1;  // This bit allows one to invert the RTS
                                     // auto flow operation polarity. If set the
                                     // RTS auto flow assert level is low* If
                                     // clear the RTS auto flow assert level is
                                     // high* RW 0x0
} aux_mu_ctrl_reg_t;
...
typedef struct {
  bool symbol_available : 1;  // If this bit is set the mini UART receive FIFO
                              // contains at least 1 symbol If this bit is clear
                              // the mini UART receiver FIFO is empty RO 0x0
  bool space_available : 1;   // If this bit is set the mini UART
                              // transmitter FIFO can accept at least one
                              // more symbol. If this bit is clear the mini
                              // UART transmitter FIFO is full RO 0x0
  bool
  ...
  bool transmitter_is_idle : 1;  // If this bit is set the transmitter
                                 // is idle. If this bit is clear the
                                 // transmitter is busy. RO 0x1
  bool receiver_overrun : 1;     // This bit is set if there was a receiver
                                 // overrun. That is: one or more characters
  // arrived whilst the receive FIFO was full. The
  // newly arrived characters have been discarded.
  // This bit is cleared each time the
  // AUX_MU_LSR_REG register is read. RO 0x0
} aux_mu_stat_reg_t;

// Mini UART Extra Status
#define AUX_MU_STAT_REG \
  ((const volatile aux_mu_stat_reg_t *const)(AUX_BASE + 0x64))

typedef struct {
  unsigned int baudrate : 16;  // mini UART baudrate counter
} aux_mu_baud_reg_t;
